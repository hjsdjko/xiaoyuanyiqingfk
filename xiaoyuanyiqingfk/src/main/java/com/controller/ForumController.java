...
public class ForumController {
    ...
    @Autowired
    private MeirixinxiService meirixinxiService;//每日信息
    @Autowired
    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(forum !=null){
            ...
            //级联表 辅导员
            //级联表
            FudaoyuanEntity fudaoyuan = fudaoyuanService.selectById(forum.getFudaoyuanId());
            if(fudaoyuan != null){
            BeanUtils.copyProperties( fudaoyuan , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yonghuId"
, "fudaoyuanId"
, "usersId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setFudaoyuanId(fudaoyuan.getId());
            }
            //级联表 防疫人员
            //级联表
    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        if(false)
            ...
            forum.setFudaoyuanId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        Wrapper<ForumEntity> queryWrapper = new EntityWrapper<ForumEntity>()
            ...
            .eq("fudaoyuan_id", forum.getFudaoyuanId())
...
//                            forumEntity.setFudaoyuanId(Integer.valueOf(data.get(0)));   //辅导员 要改的
//                            forumEntity.setYonghuId(Integer.valueOf(data.get(0)));   //防疫人员 要改的
        ...
        ForumEntity forum = forumService.selectById(id);
            if(forum !=null){
                ...
                //级联表
                    FudaoyuanEntity fudaoyuan = fudaoyuanService.selectById(forum.getFudaoyuanId());
                if(fudaoyuan != null){
                    BeanUtils.copyProperties( fudaoyuan , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setFudaoyuanId(fudaoyuan.getId());
                }
                //级联表
                    YonghuEntity yonghu = yonghuService.selectById(forum.getYonghuId());
